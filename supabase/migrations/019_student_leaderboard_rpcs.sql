-- Migration 019: Update leaderboard RPCs for student opt-out visibility
-- Students with leaderboard_visibility=false are hidden from other students
-- Teachers always see all students (regardless of visibility flag)
-- Two new RPCs: student-facing (filtered) alongside existing teacher-facing (unchanged)

-- Student-facing leaderboard: 7-day rolling, excludes opted-out students and consent-denied
CREATE OR REPLACE FUNCTION get_student_leaderboard_7d(p_teacher_id UUID)
RETURNS TABLE (
  user_id UUID,
  display_name TEXT,
  instrument TEXT,
  ...
  WHERE p.consent_status != 'denied'
    AND p.leaderboard_visibility = true
  ORDER BY total_coins_earned DESC;
$$ LANGUAGE sql STABLE SECURITY DEFINER;


-- Student-facing leaderboard: 30-day rolling, excludes opted-out students and consent-denied
CREATE OR REPLACE FUNCTION get_student_leaderboard_30d(p_teacher_id UUID)
RETURNS TABLE (
  user_id UUID,
  display_name TEXT,
  instrument TEXT,
